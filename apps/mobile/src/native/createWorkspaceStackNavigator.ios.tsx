...
import { nativeWorkspacePopCount, projectWorkspaceStack } from "./workspace-stack-projection";
import {
...
function ColumnContent(props: { readonly children: React.ReactNode }) {
  const [metrics, setMetrics] = useState<NativeLayoutMetrics | null>(null);
  return (
    <SafeAreaProvider style={{ flex: 1 }}>
      <SafeAreaView edges={["left", "right"]} style={{ flex: 1 }}>
        {/* UIKit can reserve the primary column inside the secondary view's
            safe area. Observe the inset body so consumers don't reserve it twice. */}
        <View style={{ flex: 1 }}>
...
function WorkspaceStackView(props: ViewProps) {
  ...
  const routes = [baseRoute, ...projection.overlays];
  return (
    <NativeStackView
      {...props}
      state={{ ...props.state, routes, index: routes.length - 1, preloadedRoutes: [] }}
      descriptors={{ ...props.descriptors, [baseRoute.key]: baseDescriptor }}
    />
  );
}
...
function WorkspaceStackNavigator({
  ...
  const WorkspaceView = NATIVE_WORKSPACE_COLUMNS_SUPPORTED ? WorkspaceStackView : NativeStackView;
  return (
